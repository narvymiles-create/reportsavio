...
import skavioLogoUrl from "@/assets/skavio-logo-small.png";
import "./MarksFormPage.css";

// Cache the logo as a data URL once so PDF footer renders without async hiccups
let _skavioLogoDataUrl: string | null = null;
async function getSkavioLogoDataUrl(): Promise<string | null> {
  if (_skavioLogoDataUrl) return _skavioLogoDataUrl;
  try {
    const res = await fetch(skavioLogoUrl);
    ...
    _skavioLogoDataUrl = dataUrl;
    return dataUrl;
  } catch (e) {
    console.error("[MarksFormPage] failed loading Skavio logo", e);
    return null;
  }
}

function drawSkavioFooter(doc: jsPDF, logoDataUrl: string | null) {
  const pageW = doc.internal.pageSize.getWidth();
  const pageH = doc.internal.pageSize.getHeight();
  const text = "Generated by Skavio Primary — Developed by Skavio Technologies | 0705466283";
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    const logoW = logoDataUrl ? 14 : 0;
    const gap = logoDataUrl ? 6 : 0;
    const totalW = logoW + gap + textW;
    const startX = (pageW - totalW) / 2;
    const baselineY = pageH - 14;
    if (logoDataUrl) {
      try { doc.addImage(logoDataUrl, "PNG", startX, baselineY - 11, logoW, 14); } catch {}
    }
    doc.text(text, startX + logoW + gap, baselineY);
